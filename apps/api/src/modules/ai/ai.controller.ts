...
export class AiController {
    ...
    @Post('quiz')
    async generateQuiz(@Body() body: { subject: string; topic: string; numberOfQuestions: number; context?: string }) {
        return this.aiService.generateQuizQuestions(body.subject, body.topic, body.numberOfQuestions, body.context);
    }

    @Post('flashcards')
    async generateFlashcards(@Body() body: { topic: string; numberOfCards: number; context?: string }) {
    ...
    @Post('writing')
    async analyzeWriting(@Body() body: { text: string; taskType: 'ielts_task1' | 'ielts_task2' | 'toefl_integrated' | 'toefl_independent' }) {
        return this.aiService.analyzeWriting(body.text, body.taskType);
    }


    @Post('chat')
    async chat(@Body() body: { message: string; history: { role: string; parts: string }[] }) {
        return this.aiService.chat(body.message, body.history || []);
    }

    @Post('interview')
    async interview(@Body() body: { role: string; experience: string; description: string }) {
        return this.aiService.generateInterviewQuestions(body.role, body.experience, body.description);
    }

    @Post('interview/analyze')
    async analyzeInterview(@Body() body: { question: string; transcript: string }) {
    ...
    async evaluateCoding(@Body() body: { code: string; language: string; problem: string }) {
